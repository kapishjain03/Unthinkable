...
/**
 * Mock analyzer — deterministic fallback when no API key is configured.
 */
...
/**
 * Main analysis function — orchestrates standard text or structured screenshot analysis.
 */
export async function analyzeContent(
    ...
    if (!hasApiKey) {
        console.log("No Gemini API key configured. Using mock analyzer.");
        return analyzeWithMock(text);
    }

    try {
        // If we have structured data (Vision pipeline), use the specialized analyzer
        if (structuredData && platform) {
            return await analyzeEngagement(structuredData, platform);
        }

        // Standard text analysis (PDFs or generic images)
